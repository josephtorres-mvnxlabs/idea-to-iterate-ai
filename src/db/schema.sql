...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    status TEXT NOT NULL CHECK (status IN ('backlog', 'ready', 'in_progress', 'review', 'done', 'archived')),
...
CREATE TABLE IF NOT EXISTS product_ideas (
    ...
    estimation INTEGER NOT NULL,
    priority TEXT NOT NULL CHECK (priority IN ('low', 'medium', 'high')),
    status TEXT NOT NULL CHECK (status IN ('proposed', 'under_review', 'approved', 'rejected', 'implemented', 'archived')),
    owner_id UUID NOT NULL REFERENCES users(id),
    created_by UUID NOT NULL REFERENCES users(id),
    created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
